...
#python pyrat.py --rat "https://colab.research.google.com/drive/1Q55Ye33U8yhjcuekJd7_Z4zDhNfz-O2D?usp=sharing" -md 0.0 -p 1 -x 5 -y 5 --random_seed 1
import random
moves=None
...
def traversal(start_vertex, graph):
    ...
    liste_enfant = [start_vertex]  # We   the list where we are going to store every vertex we had explored
    ...
    while len(neighbours) != 0:  # We search every accessible vertex and dont stop until we visited every vertex
        neighbours, (noeud_actuel, parent_actuel) = pop_from_structure(neighbours)  # We take the first child parent couple of the list
